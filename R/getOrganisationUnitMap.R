...
#' }
getOrganisationUnitMap<-function(organisationUnit=NA, d2session = d2_default_session) {
  if ( is.na(organisationUnit) ) { organisationUnit<-d2session$user_orgunit }
  url<-URLencode(paste0(d2session$base_url,"api/",api_version(),"/organisationUnits.json?&filter=path:like:",organisationUnit,"&fields=id,code,name,shortName&paging=false"))
  sig<-digest::digest(url,algo='md5', serialize = FALSE)
  sites<-getCachedObject(sig)
  if (is.null(sites)){
  r<-httr::GET(url,httr::timeout(600), handle = d2session$handle)
  if (r$status == 200L ){
