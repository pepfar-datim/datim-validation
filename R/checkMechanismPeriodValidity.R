...
checkMechanismPeriodValidity <- function(data) {
  ...
  bar <-
    do.call(rbind.data.frame, lapply(unique(mechs_periods$period), getPeriodFromISO))
  mechs_periods <- merge(mechs_periods, bar, by.x = "period", by.y = "iso")
  baz <-
    getMechanismsMap(organisationUnit = 
                       getOption("organisationUnit"))[, c("id", "code", "startDate", "endDate")]
  names(baz) <-
    c("attributeOptionCombo",
      ...
      "endDate_mech")
  mechs_periods <-
    merge(mechs_periods, baz, by = "attributeOptionCombo", all.x = T)
  mechs_periods$isValid <-
    mechs_periods$startDate >= mechs_periods$startDate_mech &
    mechs_periods$endDate <= mechs_periods$endDate_mech
   data_bad<-mechs_periods[!mechs_periods$isValid, ]
  if (NROW(data_bad) > 0) {
    bad_periods<-unique(data_bad$period)
    stop("Invalid mechanisms for periods found: ", paste(bad_periods,sep="",collapse=","))
  } else {
    return(TRUE)
  }
