...
#' 
getMechanismsMap<-function(base.url,username,password,organisationUnit=NA,endDate="2016-09-29") {
  ...
  url<-paste0(base.url,"api/categoryOptionCombos?filter=categoryCombo.id:eq:wUpfppgjEza&fields=code,name,id,categoryOptions[endDate]&paging=false")
  if  ( ou_filter ){
    url<-paste0(url,paste0("&filter=categoryOptions.organisationUnits.id:eq:",organisationUnit)) 
  } else {
    warning("No organisation unit specified. Assuming global")
    }
  url<-URLencode(url)
