...
getMechanismsMap<-function(organisationUnit=NA, d2session = d2_default_session) {
  ...
  url<-paste0(d2session$base_url,"api/",api_version(),"/categoryOptionCombos?filter=categoryCombo.id:eq:wUpfppgjEza&fields=code,name,id,categoryOptions[startDate,endDate]&paging=false")
  if  ( ou_filter ){
