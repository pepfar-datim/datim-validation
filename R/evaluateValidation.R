...
#'
#' @description evaluationValidation will accept a vector of combis
#' (data elements.category optioncombinations) and values
...
evaluateValidation <- function(combis,
  ...
  totals_df <- data.frame(exp = this.des,
                          values = values,
  ...
  #Empty data frame
  if (nrow(matches) == 0) {return(validation.results_empty)} #nolint
