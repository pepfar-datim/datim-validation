...
evaluateValidation<-function(combis,values,vr,return_violations_only=TRUE) {
  ...
  keep_these_rules<-(matches$leftSide.missingValueStrategy == "SKIP_IF_ANY_VALUE_MISSING" & (matches$leftSide.ops != matches$leftSide.count)) | 
    ...
    ( ( matches$rightSide.count == matches$rightSide.ops )  & (matches$rightSide.missingValueStrategy == "SKIP_IF_ALL_VALUES_MISSING" )) |
    ( ( matches$leftSide.count == matches$leftSide.ops )  & (matches$leftSide.missingValueStrategy == "SKIP_IF_ALL_VALUES_MISSING" ))
  ...
  if (nrow(matches) > 0 ) {
   ...
   #matches_ex$leftSide.expression<-matches_ex$leftSide.count > 0
   #matches_ex$rightSide.expression<-matches_ex$rightSide.count > 0
   matches_ex$formula <- paste0(as.character(matches_ex$leftSide.count == 0), matches_ex$operator,as.character(matches_ex$rightSide.count == 0)) }  else { matches_ex<-validation.results_empty }
   
  #Normal operators
  matches_normal<-matches[!(matches$operator %in% c("&","|")),]
  if (nrow(matches_normal > 0)) {
  matches_normal$leftSide.expression<-gsub(expression.pattern,"0",matches_normal$leftSide.expression) 
  matches_normal$rightSide.expression<-gsub(expression.pattern,"0",matches_normal$rightSide.expression)
  matches_normal$leftSide.expression<-sapply(matches_normal$leftSide.expression,function(x) {eval(parse(text=x))})
  matches_normal$rightSide.expression<-sapply(matches_normal$rightSide.expression,function(x) {eval(parse(text=x))})
  matches_normal$formula<-paste(matches_normal$leftSide.expression,matches_normal$operator,matches_normal$rightSide.expression) } else { matches_normal<-validation.results_empty}
