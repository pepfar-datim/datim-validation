...
DATIM accepts and to standardize this so that we can use other validation functions on the data. Refer to each functons specific documenation on how to use the functions. 
...
checkValidMechanisms(d,base.url,username,password,organisationUnit)
```


Next, we can check the indicators and disaggregations for validity against the current dataset form definitions. A data frame of indicator/disagg combinations will be returned.
...
vr_violations<-validateData(data = d,base.url = "https://www.datim.org/",
                            username = username,password = password,
                            organisationUnit = organisationUnit,return_violations_only = TRUE)
